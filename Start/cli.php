 ...
 */
\Team\System\Task::join('\team\url', function() {
   ...
   \Team\Debug::trace("Acabado el proceso de analisis CLI", $args);
...
function extract_command(& $argv) {
 return \Team\System\FileSystem::basename(array_shift($argv));
}
...
function get_store_for_args($url = null) {
	...
	if(isset($url)) {
	  return new \Team\Data\Data('Url',$url);
	}else {
	  return  new \Team\Data\Data();
	}
}
